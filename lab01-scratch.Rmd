...
```{r eval=FALSE}
filtered_trails <- trails %>% filter(class !=3) %>%
  mutate(class = if_else(is.na(class),4,class))
count(filtered_trails, class)
```
...
```{r}
ggplot(trails, aes(x = length, group = class, fill = class, colour = class)) + 
...
11. Describe the relationship between `length` and `class`. In other words, describe how the distribution of `length` compares between trails that have different classes (1 = shared use bicycle and pedestrian path, 2 = bike lane, and 3 = bike route). Include information from the graph and summary statistics from the previous exercise in your response.
    ...
    Answer: Class 1 generally have more shorter trails than class 2. (...)
...
12. The following conditions must be met when we conduct statistical inference on the difference in means between two groups. For each condition, specify whether it is met and a brief explanation of your reasoning. 
    - **Independence** Yes, each individual trails is independent from each other and no single trails is named/recorded twice.
...
H_a: \mu_1 - \mu_2 \neq 0$$
    ...
    Answer: Null hypothesis: Theere's no difference in the mean length in trails with bike     lanes (class = 2) and trials that share bikes with pedestrians (class = 1).
    Alternative hypothesis: There is difference in the mean length in trails with bike        lanes (class = 2) and trials that share bikes with pedestrians (class = 1).
...
15. Use the output from the previous exercise to answer the following: 
    ...
    - State your conclusion in the context of this analysis. Use a significance level of          $\alpha = 0.01$. 
        We would reject the null hypothsis and conclude that there's sufficient evidence            that the difference between mean length in trails with bike lanes (class = 2) and           trials that share bikes with pedestrians (class = 1) is not 0.

16. Notice the confidence interval for the difference in mean trail length printed in the output from Exercise 14. Interpret this confidence interval in the context of this analysis.     Answer: We are 99% confidence that the mean length in trails with bike lanes (class = 2)     and trials that share bikes with pedestrians (class = 1) is between -0.12 and 0.20.
