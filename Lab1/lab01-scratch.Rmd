...
1. Now that we've had a quick view of the dataset, let's get more details about its structure. Sometimes viewing a summary of the data structure is more useful than viewing the raw data, especially if the dataset has a large number of observations and/or rows. Run the code below to use the `glimpse` function to see a summary of the `trails` dataset.
    ...
    Answer: 12 variables and 739 observations.
...
ggplot(data = trails, aes(x = status)) + 
  ...
  labs(x = "Status", 
       y = "Frequency", 
       title = "Barplot for Trails Status")
```
...
ggplot(data = current_trails) +
  ...
  labs(x = "Length of trails", y = "Frequency", title = "Histogram for length of trails")

ggplot(data = current_trails, mapping = aes(x = as.factor(class), y = length)) +
  geom_boxplot() +
  labs(x = "Class", y = "Length of trails", title = "Boxplot for length of trails grouped by class")
```
...
6. Describe the distribution of `length`. Your description should include comments about the shape, center, spread, and any potential outliers. Use the graph from Exercise 4 and relevant summary statistics from Exercise 5 in your description.
    ...
    Answer: From exercise 4 and 5, since the mean is greater than median, the distribution of length is right-skewed. The center of distribution is around 0.45, and 3 outliers are identified in class 1 from the boxplot. The data of length covers a large range, around 8.4. The low standard deviation 0.85 represents that data of length are clustered around the mean.
...
```{r}
dayhikes_trails <- current_trails %>%
  filter(length <= 5)
glimpse(dayhikes_trails)
```
...
```
    ...
    Answer: Values of class in the dataset are "1", "2", "3", NA. Since there are totally 4 types of classes, most likely reason for the missing observations is that they are in class 4.
...
```{r eval=FALSE}
imputed_trails <- trails %>%
  mutate(class = if_else(is.na(class),4,class))
count(imputed_trails, class)

filtered_trails <- imputed_trails %>%
  filter(class != 3 & class != 4)
```
...
```{r}
ggplot(imputed_trails, aes(x = length, group = as.factor(class), fill = as.factor(class), colour = as.factor(class))) + 
  geom_histogram(alpha = 0.5, position = "identity", bins = 20) + 
  facet_grid(. ~ as.factor(class) ) +
  theme(legend.position = "right") + 
  labs(x = "Length of trails", title = "Distribution between the length of the trail and its class variable")

#summary statistics
tapply(imputed_trails$length, imputed_trails$class, summary)
```
...
11. Describe the relationship between `length` and `class`. In other words, describe how the distribution of `length` compares between trails that have different classes (1 = shared use bicycle and pedestrian path, 2 = bike lane, and 3 = bike route). Include information from the graph and summary statistics from the previous exercise in your response.
    ...
    Answer: There are insufficient data for class 3 and 4, therefore we cannot identify any distribution or relationship. For class 1 and 2, both distribution are right-skewed, with a larger mean than median. Also, their centers are near to each other. Length of shared use bicycle and pedestrian path (class 1) has a larger range and outliers than length of bike lane (class 2). There are much more shared path (class 1) collected in the sample.
...
12. The following conditions must be met when we conduct statistical inference on the difference in means between two groups. For each condition, specify whether it is met and a brief explanation of your reasoning. 
    - **Independence** Yes, each individual trails is independent from each other. Although they might be connected, their lengths aren't related to each other.
    - **Sample Size** Yes, we have 729 samples for class 1 and 2, which is considered a large sample size.
    - **Independent Groups** Yes, each trails is assigned to only one type of class, therefore two classes are independent from each other.
...
H_a: \mu_1 - \mu_2 \neq 0$$
    ...
    Answer: Null hypothesis: There's no difference in the mean length in trails with bike lanes (class = 2) and trials that share bikes with pedestrians (class = 1).
    Alternative hypothesis: There is a difference in the mean length in trails with bike lanes (class = 2) and trials that share bikes with pedestrians (class = 1).
...
t.test(length ~ class, data = filtered_trails, 
...
15. Use the output from the previous exercise to answer the following: 
    - Write the definition of the test statistic in the context of this analysis.
        The number of standard errors of the difference in mean length in trails with bike lanes (class = 2) and trials that share bikes with pedestrians (class = 1) is 0.60392. 
        
    - Write the definition of the p-value in the context of this analysis. 
        Assuming there's no difference between mean length in trails with bike lanes (class = 2) and trials that share bikes with pedestrians (class = 1), the probability of obtaining test results as least as extreme as the result actually observed is 0.5462.

    - State your conclusion in the context of this analysis. Use a significance level of          $\alpha = 0.01$. 
        Since p-value is larger than 0.01, We conclude that there isn't sufficient evidence against the null hypothsis and fail to reject that there's no difference between mean length in trails with bike lanes (class = 2) and trials that share bikes with pedestrians (class = 1) is not 0.
...
16. Notice the confidence interval for the difference in mean trail length printed in the output from Exercise 14. Interpret this confidence interval in the context of this analysis.     
    Answer: We are 99% confidence that the difference of mean length in trails with bike lanes (class = 2) and trials that share bikes with pedestrians (class = 1) is between -0.12 and 0.20.
